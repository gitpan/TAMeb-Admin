...
sub create {
    ...
    $user = TAMeb::Admin::User->new( $tam, name => $name );
    if ( $user->exist ) {
    ...
    $resp = TAMeb::Admin::User->list($tam, pattern => $finddn, bydn => 1);
    if ( $resp->isok ) {
...
sub list {
    ...
    $resp = TAMeb::Admin::User->list($tam, pattern => $pattern, maxreturn => $maxreturn, bydn => $bydn);
    unless ( $resp->isok ) {
...
sub show {
    ...
    if ( $bydn ) {
	$user = TAMeb::Admin::User->new( $tam, dn => $name );
    }
    else {
	$user = TAMeb::Admin::User->new( $tam, name => $name );
    }
...
sub show_groups {
    ...
    my $user = TAMeb::Admin::User->new( $tam, name => $name );
    unless ( $user->exist ) {
	print "Cannot find TAM user \"$name\"\n";
	return 2;
    }
