...
sub create {
    ...
    my $acl  = TAMeb::Admin::ACL->new( $tam, name => $name );
    if ( $action eq 'create' ) {
	if ( $acl->exist ) {
...
sub show {
    ...
    $acl  = TAMeb::Admin::ACL->new( $tam, name => $name );
...
sub _remove {
	...
	if ( $target eq 'user' ) {
	    $resp = TAMeb::Admin::User->list( $tam, pattern => $tname );
	    ...
	    $resp = TAMeb::Admin::Group->list( $tam, pattern => $tname );
	    unless ( $resp->isok and $resp->value ) {
