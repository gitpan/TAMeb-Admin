package TAMeb::PDAdmin::objectspace;
use strict;
use warnings;
...
sub create {
    ...
    my $ospace = TAMeb::Admin::Objectspace->new($tam, 
					      name => $name, 
					      type => $type || 0, 
					      description => $desc );
...
sub delete {
    ...
    my $ospace = TAMeb::Admin::Objectspace->new($tam, name => $name);
...
sub list {
    ...
    my $resp = TAMeb::Admin::Objectspace->list($tam);
    unless ( $resp->isok ) {
