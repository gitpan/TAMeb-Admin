...
sub create {
    ...
    $web = TAMeb::Admin::SSO::Web->new($tam,name=>$name,description=>$desc);
    if ( $action eq 'create' ) {
	if ( $web->exist ) {
...
sub list {
    ...
    $resp = TAMeb::Admin::SSO::Web->list($tam);
    if ( $resp->isok ) {
	for ( sort $resp->value ) {
	    print "    $_\n" if /^$name/;
	}
...
sub show {
    ...
    $web  = TAMeb::Admin::SSO::Web->new( $tam, name => $name );
