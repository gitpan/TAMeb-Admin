...
sub create {
    ...
    my $pop  = TAMeb::Admin::POP->new( $tam, name => $name );
    if ( $action eq 'create' ) {
	if ( $pop->exist ) {
...
sub show {
    ...
    $pop  = TAMeb::Admin::POP->new( $tam, name => $name );
...
sub modify {
    ...
    my %mod_dispatch = (
	description   => \&TAMeb::Admin::POP::description,
	qop	      => \&TAMeb::Admin::POP::qop,
	'audit-level' => \&_audit,
	'tod-access'  => \&_tod,
	warning       => \&_warnmode,
	ipauth	      => \&_ipauth,
...
sub detach {
    ...
    my $obj = TAMeb::Admin::ProtObject->new( $tam, name => $object );

    # I want to find out which pop is attached to the named object
    $resp = $obj->pop;
    if ( $resp->value->{attached} ) {
	my $pop = TAMeb::Admin::POP->new($tam, name => $resp->value->{attached});
