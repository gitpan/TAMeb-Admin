package TAMeb::PDAdmin::TabComplete::utils;
use strict;
use warnings;
...
sub _list_group_or_user {
    ...
    $resp = $func eq 'group' ? TAMeb::Admin::Group->list($tam) : TAMeb::Admin::User->list($tam);
    if ( $resp->isok ) {
...
sub _listObj {
    ...
    my $protobj = TAMeb::Admin::ProtObject->new( $tam, name => $prefix );

    my $resp = $protobj->list;

    if ( $resp->isok ) {
	for my $sublink ( grep { m#^$word# } $resp->value ) {
	    my $subobj = TAMeb::Admin::ProtObject->new( $tam, name => $sublink );
	    $resp = $subobj->type;
...
sub _guessDN {
	...
	my $resp = $type eq 'group' ? TAMeb::Admin::Group->list($tam, bydn => 1 ) :
				      TAMeb::Admin::User->list($tam, bydn => 1 );
...
sub _list_gso {
    ...
    $resp = $func eq 'web' ? TAMeb::Admin::SSO::Web->list($tam) :
			     TAMeb::Admin::SSO::Group->list($tam);
#    $resp = TAMeb::Admin::SSO::Group->list($tam);
    if ( $resp->isok ) {
	return grep { /^$word/ } $resp->value;
    }
    else {
