...
sub create {
    ...
    $resp = TAMeb::Admin::Group->list($tam, pattern => $finddn, bydn => 1);
    if ( $resp->isok ) {
    ...
    $group = TAMeb::Admin::Group->new( $tam, name => $name );
    if ( $group->exist ) {
...
sub list {
    ...
    $resp = TAMeb::Admin::Group->list($tam, pattern => $pattern, maxreturn => $maxreturn, bydn => $bydn);
    unless ( $resp->isok ) {
...
sub show {
    ...
    if ( $bydn ) {
	$grp = TAMeb::Admin::Group->new( $tam, dn => $name );
    }
    else {
	$grp = TAMeb::Admin::Group->new( $tam, name => $name );
    }
...
sub show_members {
    ...
    my $grp = TAMeb::Admin::Group->new( $tam, name => $name );
    unless ( $grp->exist ) {
	print "Cannot find TAM group \"$name\"\n";
	return 2;
    }
