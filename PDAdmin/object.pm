...
sub create {
    ...
    my $pobj = TAMeb::Admin::ProtObject->new($tam, 
					   name => $name, 
					   type => $type || 0, 
					   description => $desc );
    if ( $pobj->exist ) {
...
sub delete {
    ...
    my $pobj = TAMeb::Admin::ProtObject->new($tam, name => $name);
    unless ( $pobj->exist ) {
...
sub list {
    ...
    $pobj = TAMeb::Admin::ProtObject->new($tam, name => $name);
    unless ($pobj->exist) {
	print "Object $name does not exist\n";
	help('list');
	return 2;
