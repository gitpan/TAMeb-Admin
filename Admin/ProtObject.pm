package TAMeb::Admin::ProtObject;
use strict;
use warnings;
use Carp;
use Data::Dumper;
...
#-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
# $Id: ProtObject.pm 296 2006-09-27 15:22:32Z mik $
#-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
$TAMeb::Admin::ProtObject::VERSION = '0.06';
use Inline(C => 'DATA',
		INC  => '-I/opt/PolicyDirector/include',
                LIBS => ' -lpthread  -lpdadminapi -lstdc++',
		CCFLAGS => '-Wall',
		VERSION => '0.06',
		NAME => 'TAMeb::Admin::ProtObject',
	    );
...
=head1 SYNOPSIS
  ...
  my $resp;
  my $pd = TAMeb::Admin->new( password => 'foobar' );
  my $pobj = TAMeb::Admin::ProtObject->new( $pdadmin, 
					  name => '/test/monkey');
  ...
  my $top = TAMeb::Admin::ProtObject->new( $pd, name => '/test' );
  $resp = $top->list;
...
B<TAMeb::Admin::ProtObject> provides the interface to the protected object API
calls.
...
Creates a new L<TAMeb::Admin::ProtObject> object.
...
A blessed and initialized L<TAMeb::Admin::Context>.  This is the only required
parameter.
...
A blessed L<TAMeb::Admin::ProtObject> object.  If the type is specified and it
...
L<TAMeb::Admin::ProtObject> instance and create the object in the policy
database.

In this case, the newly created instance will be returned to you in a
L<TAMeb::Admin::Response> object.  See that module's Fine Documentation to learn
how to get it.
...
A L<TAMeb::Admin::Response> object containing the new instance.
...
for L<TAMeb::Admin::ACL> and L<TAMeb::Admin::Authzrule> make more sense.
...
A L<TAMeb::Admin::Response> object containing a possibly empty array of all
objects found.


=head1 METHODS

Unless otherwise mentioned, everything returns a L<TAMeb::Admin::Response>
object.
...
Returns the attached and effective POP.  See L<TAMeb::Admin::POP> for the attach
...
a L<TAMeb::Admin::Response> object.

=head2 exist

Returns a boolean indicating if the object exists or not.  This does B<not>
return a L<TAMeb::Admin::Response> object.

=head1 ACKNOWLEDGEMENTS

See L<TAMeb::Admin> for the full list of acknoledgements.
