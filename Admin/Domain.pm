...
#-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
# $Id: Domain.pm 296 2006-09-27 15:22:32Z mik $
#-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=

$TAMeb::Admin::Action::VERSION = '0.06';
use Inline(C => 'DATA',
		INC  => '-I/opt/PolicyDirector/include',
                LIBS => ' -lpthread  -lpdadminapi -lstdc++',
		CCFLAGS => '-Wall',
		VERSION => '0.06',
		NAME => 'TAMeb::Admin::Domain',
	  );
...
sub create { 
	...
	unless ( UNIVERSAL::isa($pd, 'TAMeb::Admin::Context') ) {
...
=head1 SYNOPSIS
  ...
  my $dom = TAMeb::Admin::Domain->new( $pd,
				     name => 'Test',
				     admin => 'chimchim',
  ...
  $resp = TAMeb::Admin::Domain->create( $pd,
				      name => 'Test1',
				      admin => 'chimchim',
  ...
  $resp = $dom->list;
  # Or
  $resp = TAMeb::Admin::Domain->list($pd);
...
Creates a blessed L<TAMeb::Admin::Domain> object.  You will need to destroy this
...
A blessed L<TAMeb::Admin::Domain> object. 

=head2 create( PDADMIN, name =E<gt> NAME, admin =E<gt>  ADMINID, description =E<gt> DESC, password =E<gt> PASSWORD )

Instantiates a L<TAMeb::Admin::Domain> object and creates the domain in 
...
A blessed L<TAMeb::Admin::Domain> object, buried in a L<TAMeb::Admin::Response>
object.
...
A fully initialized L<TAMeb::Admin::Context> object.
...
in the method's signature -- the L<TAMeb::Admin::Context> object is no longer
required.
...
The domain's name.  This is NOT buried in a L<TAMeb::Admin::Response> object.
