...
#-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
# $Id: ACL.pm 296 2006-09-27 15:22:32Z mik $
#-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
$TAMeb::Admin::ACL::VERSION = "0.06";
use Inline( C => 'DATA',
		 INC  => '-I/opt/PolicyDirector/include',
                 LIBS => ' -lpthread  -lpdadminapi -lstdc++',
		 CCFLAGS => '-Wall',
		 VERSION => '0.06',
		 NAME => 'TAMeb::Admin::ACL');
...
sub find {
    ...
    return TAMeb::Admin::ProtObject->find( $pd, acl => $self->name );
}
...
sub create {
	...
	unless (defined($pd) and UNIVERSAL::isa($pd,'TAMeb::Admin::Context')){
	    $resp->set_message("Invalid TAMeb::Admin::Context object");
	    $resp->set_isok(0);
	    return $resp;
	}
	$self = $self->new( $pd, @_ );
...
=head1 SYNOPSIS
    ...
    my $tam  = TAMeb::Admin->new( password => 'foobar' );
    my ($resp,$rc);

    # See what ACLs exist
    $resp = TAMeb::Admin::ACL->list($tam);
    print join("\n", $resp->value);

    # Create a new ACL
    my $acl = TAMeb::Admin::ACL->new( $tam );
    ...
    $acl = TAMeb::Admin::ACL->new($tam, 'default-webseal');
...
L<TAMeb::Admin::ACL|TAMeb::Admin::ACL> provides the interface to the ACL portion of the TAM
Admin API.  
...
Creates a blessed B<TAMeb::Admin::ACL> object and returns it.
...
An initialized L<TAMeb::Admin::Context|TAMeb::Admin::Context> object.  Please note that, after the
L<TAMeb::Admin::ACL|TAMeb::Admin::ACL> object is created, you cannot change the context w/o
...
A fully blessed L<TAMeb::Admin::ACL|TAMeb::Admin::ACL> object.
...
L<TAMeb::Admin::ACL|TAMeb::Admin::ACL> object.  If it wasn't successful, you will
get an error message why.  If the object already exists, you will get a
warning and the L<TAMeb::Admin::ACL|TAMeb::Admin::ACL> object.  And all of this
will be embedded in a L<TAMeb::Admin::Response|TAMeb::Admin::Response> object.

=head1 CLASS METHODS

Class methods behave like instance methods -- they return
L<TAMeb::Admin::Response|TAMeb::Admin::Response> objects.
...
A fully blessed L<TAMeb::Admin::Context|TAMeb::Admin::Context> object.  
...
All of the methods return a L<Tam::Admin::Response|TAMeb::Admin::Response> object.  See the
...
calls will embed the results of a 'get' in the L<TAMeb::Admin::Response|TAMeb::Admin::Response> object.
...
just a wrapper for L<TAMeb::Admin::ProtObject|TAMeb::Admin::ProtObject>.  I like
...
Please read L<TAMeb::Admin|TAMeb::Admin> for the full list of acks.  I stand upon the
shoulders of giants.
