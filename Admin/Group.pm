package TAMeb::Admin::Group;
use strict;
use warnings;
use Carp;

#-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
# $Id: Group.pm 296 2006-09-27 15:22:32Z mik $
#-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
$TAMeb::Admin::Group::VERSION = '0.06';
use Inline( C => 'DATA',
	        INC  => '-I/opt/PolicyDirector/include',
                LIBS => ' -lpthread  -lpdadminapi -lstdc++',
		CCFLAGS => '-Wall',
		VERSION => '0.06',
	        NAME   => 'TAMeb::Admin::Group',
		);
...
sub list {
    ...
    # I want this to be called as either TAMeb::Admin::Group->list of
    # $self->list
    if ( ref $class ) {
	$pd = $class->{context};
    }
...
=head1 SYNOPSIS
    ...
    $resp = TAMeb::Admin::Group->list( $pd, pattern => "lgroup*" );
    print join( "\n", @{$resp->value} );
    # Alternately, search by DN.
    $resp = TAMeb::Admin::Group->list( $pd, pattern => "lgroup*", bydn => 1 );
    print join( "\n", @{$resp->value} );

    # Create a new group the easy way
    $resp = TAMeb::Admin::Group->create( $pd, 
					name => 'lgroup',
	...
	$groups[$i] = TAMeb::Admin::Group->new( $pd, name => $name );
...
B<TAMeb::Admin::Group> provides the interface to the group portion of the TAM API.

=head1 CONSTRUCTOR

=head2 new( PDADMIN[, name=E<gt> NAME, dn =E<gt> DN, cn =E<gt> CN] )

Creates a blessed B<TAMeb::Admin::Group> object and returns it.
...
L<TAMeb::Admin::Group> object is created, you cannot change the context w/o destroying
...
A fully blessed L<TAMeb::Admin::Group> object.

=head1 CLASS METHODS

Class methods behave like instance methods -- they return
L<TAMeb::Adming::Response> objects.
...
L<TAMeb::Admin::Response> object, I am not going to worry about documenting that
...
from the L<TAMeb::Admin::Response> object.
...
The success of the ooerstion if B<new> was used, the new L<TAMeb::Admin::Group>
object otherwise.
...
create a new L<TAMeb::Admin::Group> object.
...
L<TAMeb::Admin::Group> object otherwise.

=head1 ACKNOWLEDGEMENTS

See L<TAMeb::Admin>.  I stand upon the shoulders of giants.
