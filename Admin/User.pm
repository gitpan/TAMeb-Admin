...
#-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
# $Id: User.pm 296 2006-09-27 15:22:32Z mik $
#-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
$TAMeb::Admin::User::VERSION = '0.06';
use Inline(C => 'DATA',
		INC  => '-I/opt/PolicyDirector/include',
                LIBS => ' -lpthread  -lpdadminapi -lstdc++',
		CCFLAGS => '-Wall',
		VERSION => '0.06',
		NAME => 'TAMeb::Admin::User',
	  );
...
sub create {
    ...
    unless ( ref $self ) {
	my $pd = shift;
	unless ( defined($pd) and UNIVERSAL::isa($pd,'TAMeb::Admin::Context' ) ) {
...
sub list {
    ...
    # I want this to be called as either TAMeb::Admin::User->list or
    # $self->list
    if ( ref($class) ) {
	$pd = $class->{context};
    }
...
sub groups {
	...
	for my $grp ( @{$opts{remove}} ) {
		...
		$group = TAMeb::Admin::Group->new( $self->{context}, name => $grp );
	    }
	    my $gname = $group->name;
...
=head1 SYNOPSIS
  ...
  $resp = TAMeb::Admin::User->list( $pd, pattern => "luser*", maxreturn => 0 );
  print join("\n", $resp->value);

  # Lets search by DN instead
  $resp = TAMeb::Admin::User->list( $pd, pattern => "luser*", maxreturn => 0, bydn => 1 );
  print join("\n", $resp->value);

  # Lets create three new users, the easy way
  for my $i ( 0 .. 2 ) {
      my $name = sprintf "luser%02d", $i;
      $resp = TAMeb::Admin::User->create( $pd, name => $name,
  ...
  push @users, TAMeb::Admin::User->new( $pd, name => 'luser03',
...
L<TAMeb::Admin::User> implements the User portion of the TAM API.  There is a fair
amount of overlap between L<TAMeb::Admin::User> and L<TAMeb::Admin::Context>.  Since I am a
...
Creates a blessed L<TAMeb::Admin::User> object.  As with everything else, you will need
...
An initialized L<TAMeb::Admin::Context> object.  This is the only required
parameter.
...
A fully blessed L<TAMeb::Admin::User> object, with an embedded context.

=head1 CLASS METHODS

Class methods behave like instance methods -- they return
L<TAMeb::Admin::Response> objects.
...
A fully blessed L<TAMeb::Admin::Context> object.  Since this is a class method,
and L<TAMeb::Admin::Context> objects are stored in the instances, you must
provide it.
...
All of the methods return a L<TAMeb::Admin::Response> object.  See the
...
L</"new"> and retrieve the L<TAMeb::Admin::User> object out of the Response object.
...
L<TAMeb::Admin::User> object if not.  If the name, the DN, the CN and the SN are not
...
L</"create">, you can call this method to initialize the L<TAMeb::Admin::User> object.
...
L</"new">, you will get the L<TAMeb::Admin::User> object.
...
message from the L<TAMeb::Admin::Response> object and the iswarning flag will be
...
These are identical to the same named functions in L<TAMeb::Admin::Context>.
...
See L<TAMeb::Admin> for the list.  This was not possible without the help of a
bunch of people smarter than I.

=head1 BUGS

Should L</"list"> return a list of names, or a list of L<TAMeb::Admin::User>
objects? 
