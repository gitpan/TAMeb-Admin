...
sub _get_credtype {
    ...
    my $test = TAMeb::Admin::SSO::Web->new( $tam, name => $cred );
    if ( $test->exist ) {
	return 'web';
    }
    else {
	$test = TAMeb::Admin::SSO::Group->new( $tam, name => $cred );
	if ( $test->exist ) {
	    return 'group';
	}
    }
...
sub list {
    ...
    if ( $resp->isok ) {
	for ( @hrefs ) {
	    push @rc,TAMeb::Admin::SSO::Cred->new($pd, %{$_}); 
	}
	$resp->set_value(\@rc);
    }
    return $resp;
...
=head1 SYNOPSIS
    ...
    my $sso = TAMeb::Admin::SSO::Cred->new( $pd,
					resource => 'fred',
					uid  => 'mik',
					ssouid => 'mikfire',
					ssopwd => 'pa$$w0rd',
    ...
    $resp = $sso->list();
    for ( $resp->value ) {
	isa_ok($_, "TAMeb::Admin::SSO::Cred");
    }

=head1 DESCRIPTION

L<TAMeb::Admin::SSO::Cred> provides the interface to create and modify GSO
credentials.
...
Creates a blessed L<TAMeb::Admin::SSO::Cred> object.  
...
An initialized L<TAMeb::Admin::Context> object.  As with every other class, the
only way to change the context is to destroy the L<TAMeb::Admin::SSO::Cred>
...
password is stored in plain text in the L<TAMeb::Admin::SSO::Cred> object.  This
...
A fully blessed L<TAMeb::Admin::SSO::Cred> object under normal circumstances,
...
Initializes the L<TAMeb::Admin::SSO::Cred> and creates it in TAM as well.
...
A L<TAMeb::Admin::Response> object indicating the success or failure of the
create operation.  If it could be created, the new L<TAMeb::Admin::SSO::Cred>
...
An initialized L<TAMeb::Admin::Context> object.  
...
A list of initialized L<TAMeb::Admin::SSO::Cred> objects, one for each GSO
credential the user has.  This list may be empty.  Please do note that this is
different from every other list method in TAMeb::Admin.

This list is, of course, embedded in a L<TAMeb::Admin::Response> object.

=head1 METHODS

The standard disclaimer.  All the methods will return a
L<TAMeb::Admin::Response> object unless specifically stated otherwise.  See the
...
will embed the results of a 'get' in the L<TAMeb::Admin::Response> object.
