...
=head1 SYNOPSIS
    ...
    $resp = TAMeb::Admin::SSO::Web->list($pd);
    print join("\n", $resp->value);
    
    # Create the web SSO resource if it doesn't exist
    $sso = TAMeb::Admin::SSO::Web->new($pd) unless $sso->exist;
...
L<TAMeb::Admin::SSO::Web> provides the interface to create and manage web GSO
resources.
...
Creates a blessed L<TAMeb::Admin::SSO::Web> object and returns it.
...
only way to change the context is to destroy the L<TAMeb::Admin::SSO::Web>
...
A blessed L<TAMeb::Admin::SSO::Web> object if things worked.  undef will be
...
A L<TAMeb::Admin::Response> object, containing the L<TAMeb::Admin::SSO::Web>
...
in an L<TAMeb::Admin::Response> object.
...
embedded in an L<TAMeb::Admin::Response> object.  You will get an empty string
...
True if the resource exists, false otherwise.  Again, this is not embedded in an L<TAMeb::Admin::Response> object.
