...
my $sso = TAMeb::Admin::SSO::Cred->new( $pd, resource => 'twiki' );
isa_ok($sso, "TAMeb::Admin::SSO::Cred");

my ($resp,$web,$user);
# Some prework -- I need a resource before I can do the sso
$resp = TAMeb::Admin::SSO::Web->create( $pd, name => 'fred', 
...
$sso = TAMeb::Admin::SSO::Cred->new( $pd,
				    resource => 'fred',
				    uid  => 'mik',
				    type => 'web',
				    ssouid => 'mikfire',
...
$sso = TAMeb::Admin::SSO::Cred->new($pd, resource => 'fred');
isa_ok($sso, "TAMeb::Admin::SSO::Cred");
...
$resp = TAMeb::Admin::SSO::Cred->create( $pd,
				    resource => 'fred',
				    uid  => 'mik',
				    type => 'web',
				    ssouid => 'mikfire',
...
$resp = TAMeb::Admin::SSO::Cred->list($pd, "mik");
...
$resp = TAMeb::Admin::SSO::Cred->list($pd, uid => "mik");
...
$resp = TAMeb::Admin::SSO::Cred->list( $pd );
is($resp->isok, 0, "Could not call list without a uid");

$resp = TAMeb::Admin::SSO::Cred->list( $pd, qw/one two three/ );
is($resp->isok, 0, "Could not call list with an odd number of parameters");

$resp = TAMeb::Admin::SSO::Cred->list($pd,foobar => 'derf');
