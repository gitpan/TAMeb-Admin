...
my $acl = TAMeb::Admin::ACL->new( $pd, name => 'monkey' );
my ($resp,@temp);
my $limit = "Trx";

isa_ok( $acl, 'TAMeb::Admin::ACL' );
...
$resp = TAMeb::Admin::ACL->list($pd);
ok( $resp->isok, "Listed ACLs as a class method");
is_deeply( [$resp->value], \@listacl, "and got the same list as before");

print "\nTESTING new\n";
$acl = TAMeb::Admin::ACL->new( $pd );
isa_ok( $acl, 'TAMeb::Admin::ACL', "Created an ACL with no name" );

$acl = TAMeb::Admin::ACL->new( $pd, 'monkey1' );
isa_ok( $acl, 'TAMeb::Admin::ACL', "Created another monkey" );
...
$acl = TAMeb::Admin::ACL->new( $pd, 'default-webseal' );
$resp = $acl->find;
...
print "\nTESTING broken new\n";
$acl = TAMeb::Admin::ACL->new();
is($acl, undef, 'No parameters whatsoever failed');

$acl = TAMeb::Admin::ACL->new( 'wikiwikiwik' );
is($acl, undef, 'No TAMeb::Admin::Context failed');

$acl = TAMeb::Admin::ACL->new( $pd, 'monkey1', 'monkey2', 'monkey3' );
is( $acl, undef, "Too many parameters failed");

$acl = TAMeb::Admin::ACL->new( $pd, name => 0 );
is($acl->exist, 0, "Unnamed ACL worked");

print "\nTESTING broken create\n";
$resp = TAMeb::Admin::ACL->create();
is($resp->isok, 0, 'No parameters whatsoever failed');

$acl = TAMeb::Admin::ACL->create( 'wikiwikiwik' );
is($resp->isok, 0, 'No TAMeb::Admin::Context failed');

$resp = TAMeb::Admin::ACL->create( $pd, 'monkey1' );
