...
my $rule = TAMeb::Admin::AuthzRule->new($pd);
isa_ok( $rule, 'TAMeb::Admin::AuthzRule' );
is( $rule->exist, 0, 'The test rule does not exist' ) or diag(Dumper($rule));

$rule = TAMeb::Admin::AuthzRule->new( $pd, name => 'Test' );
isa_ok( $rule, 'TAMeb::Admin::AuthzRule' );
is( $rule->exist, 0, 'The test rule does not exist' ) or diag(Dumper($rule));

print "\nTesting creation\n";

my $resp = TAMeb::Admin::AuthzRule->create($pd, name => 'Test', rule => $ruletext);
is( $resp->isok, 1, "Used create as a constructor" );
isa_ok( $resp->value, "TAMeb::Admin::AuthzRule", "and got the right object class");
$rule = $resp->value;
...
for ( 0 ... 4 ) {
    ...
    $resp = TAMeb::Admin::AuthzRule->create($pd,name => $temp ,rule => $ruletext);
    $rules[$_] = $resp->value;
}

$resp = TAMeb::Admin::AuthzRule->list($pd);
@temp = $resp->value;
...
$resp = TAMeb::Admin::AuthzRule->list($pd, pattern => '*\d$');
@temp = $resp->value;
...
my $newrule = TAMeb::Admin::AuthzRule->new($pd, name => 'Test');
...
$foo = TAMeb::Admin::AuthzRule->new($pd, sillybastard => 'borked' );
