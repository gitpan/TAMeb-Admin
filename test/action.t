...
my $act = TAMeb::Admin::Action->new( $pd, actionid    => 'Z',
					description => 'Test action',
				        type        => 'Test action');
isa_ok( $act, 'TAMeb::Admin::Action' );
$resp = $act->create;
...
$act = TAMeb::Admin::Action->new( $pd, actionid    => 'Z',
				     type        => 'Test action');
$resp = $act->create; 
...
$act = TAMeb::Admin::Action->new( $pd, type    => 'Action test',
				     description => 'Test action' );
...
$resp = TAMeb::Admin::Action->create( actionid    => 'Z',
				    description => 'Test action',
				    type        => 'Test action');
is( $resp->isok, 0, "Couldn't create w/o context" ) or diag($resp->messages);
$act = $resp->value;

$resp = TAMeb::Admin::Action->create( $pd,actionid    => 'Z',
					description => 'Test action',
				        type        => 'Test action');
...
$resp = TAMeb::Admin::Action->group($pd);
is( $resp->isok, 1, "Got a list of action groups" )
    or diag($resp->messages);
my @groups = $resp->value;

$resp = TAMeb::Admin::Action->group(create => "Test" );
is( $resp->isok, 0, "Couldn't create a group w/o a context" )
    or diag( $resp->messages );

$resp = TAMeb::Admin::Action->group($pd, create => "Test" );
...
$resp = TAMeb::Admin::Action->group($pd, remove => "Test" );
...
$resp = TAMeb::Admin::Action->create( $pd, 
			       actionid => 'Z',
...
$resp = TAMeb::Admin::Action->group($pd, remove => $newgrp);
...
$resp = TAMeb::Admin::Action->list($pd,'one');
