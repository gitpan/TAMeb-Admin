...
$resp = TAMeb::Admin::User->create( $pd, 
				   name     => 'luser01',
...
$resp = $user->delete();
$resp = TAMeb::Admin::User->userimport( $pd,
				      name => 'luser01',
				      dn   => 'cn=luser01,ou=people,o=me,c=us',
				      sso  => 1,
				      groups => [qw/iv-admin/],
...
$user = TAMeb::Admin::User->new($pd);
isa_ok($user, "TAMeb::Admin::User", 'Called new with no parameters');
...
$resp = TAMeb::Admin::User->list( $pd, pattern => 'luser*' );
is_deeply( [$resp->value], \@names, "Found all the lusers by class" ) or diag( Dumper($resp->value) );

$resp = TAMeb::Admin::User->list( $pd, pattern => 'luser*', maxreturn => 1);
is( scalar($resp->value), 1, "Returned just one user");

$resp = TAMeb::Admin::User->list( $pd, maxreturn => 1);
is( scalar($resp->value), 1, "Returned just one user from a * search");

$resp = TAMeb::Admin::User->list( $pd, maxreturn => 1, bydn => 1);
